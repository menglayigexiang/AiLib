...
int main(int argc, char* argv[])  // 手动验证 Kimi Messages 普通 Chat 和一次无副作用工具往返
{
    ...
    config.baseUrl = QUrl(QStringLiteral("https://api.kimi.com/coding/"));
    ...
    if (config.apiKey.isEmpty()) { log << "missing KIMI_CODE_API_KEY\n"; return 2; }
    ...
    chat.model = QStringLiteral("kimi-for-coding");
